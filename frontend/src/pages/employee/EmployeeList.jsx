...
export default function EmployeeList() {
  const { data: rawEmployees, isLoading, refetch } = useEmployees();
  ...
  const [selectedEmployeeForDrawer, setSelectedEmployeeForDrawer] =
    useState(null);
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((e) => {
      if (
        ...
        e.ministryId !== Number(selectedMinistryId)
      )
        return false;
      ...
      if (
        ...
        e.employmentType !== selectedEmploymentType
      )
        return false;
      if (selectedPayGrade !== "all" && e.payGrade !== selectedPayGrade)
        return false;
      ...
      if (search.trim() !== "") {
        ...
        const codeMatch = e.employeeCode
          ...
          : false;
        const desigMatch = e.designation
  ...
  const handleDeactivate = (emp) => {
    if (
      window.confirm(`Are you sure you want to deactivate officer ${emp.name}?`)
    ) {
  ...
  if (isLoading)
    return <div style={{ height: "400px" }} className="skeleton-shimmer"></div>;
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
          ...
          <div>
            ...
            >
              Central Bangladesh Government Officer Directory & Personnel
              Management Roster
            </p>
          </div>
        </div>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                color:
                  viewMode === "grid" ? "var(--primary)" : "var(--slate-muted)",
                display: "flex",
                alignItems: "center",
                gap: "0.25rem",
                fontWeight: 700,
      ...
      {viewMode === "table" ? (
        <div className="card-base" style={{ padding: "1.25rem" }}>
          {filteredEmployees.length === 0 ? (
            <EmptyState
              ...
              description="No employee profiles match the selected search & filter criteria."
            />
          ) : (
                      ...
                      <td style={{ textAlign: "center" }}>
                        ...
                        >
                          ...
                          <button
                            ...
                            title="View Officer Details Drawer"
                          ...
                          <button
                            onClick={() => handleOpenDrawer(emp)}
                            title="Attendance Status & Device"
                            className="btn btn-ghost"
                            style={{
                              padding: "0.25rem 0.4rem",
                              fontSize: "0.75rem",
        ...
        >
          {filteredEmployees.map((emp) => (
            <div
              key={emp.id}
              className="card-base hover-card-elevation"
              style={{
                padding: "1.25rem",
                display: "flex",
                flexDirection: "column",
              ...
              >
                ...
                >
                  ...
                  <strong style={{ color: "#059669" }}>
                    {emp.payGrade} ({formatBDT(emp.monthlySalary)})
                  </strong>
                </div>
              </div>
